...
function CreateUserModal({ onClose, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    setError(null)
    try {
      await adminUsersApi.create({ email, password, isNurse, role, isActive })
      onCreated()
    } catch (err) {
      setError(err)
    } finally {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-field">
          <label className="form-label">
            Email / Username<span className="form-label__required">*</span>
          </label>
          <input
...
function EditUserModal({ user, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    setError(null)
    try {
      await adminUsersApi.update(user.id, { email, isNurse, role, isActive })
      onSaved()
    } catch (err) {
      setError(err)
    } finally {
    ...
    <Modal title={`Edit User — ${user.email}`} onClose={onClose}>
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-field">
          ...
          <input className="form-input" value={email} onChange={(e) => setEmail(e.target.value)} autoFocus />
        </div>
        <div className="form-field" style={{ marginTop: 14 }}>
          <label className="form-label">Role</label>
          <select className="form-select" value={role} onChange={(e) => setRole(e.target.value)}>
